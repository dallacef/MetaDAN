...
def plot_pca(f, l, feat_sel):
    ...
    for i in np.unique(l):
        ax.scatter(ft[l == i, 0], ft[l == i, 1], c=l[l == i], s=200, cmap=plt.cm.jet, vmin=min(l), vmax=max(l),
                   edgecolors='None', alpha=0.6)
        ax.text(ft[l == i, 0].mean(), ft[l == i, 1].mean(), 'Class ' + str(i), horizontalalignment='center',
...
if __name__ == "__main__":
    ...
    f = pd.read_csv(par['inp_f'], sep='\t', header=None, index_col=0, dtype=str)
    f = f.T
    ...
    if par['define']:
        ...
        for i in range(len(d)):
            l[(f[d.iloc[i, 1]].isin(d.iloc[i, 2:])).tolist()] = int(d.iloc[i, 0])
    else:
    ...
    for j in range(runs_n):
        ...
        if par['target']:
            ...
            if par['set_seed']:
                skf = StratifiedKFold(n_splits=runs_cv_folds, shuffle=True, random_state=j)
            else:
                skf = StratifiedKFold(n_splits=runs_cv_folds, shuffle=True)

            for i, (train_index, test_index) in enumerate(
                    skf.split(X=np.zeros(len(l.iloc[i_u.values.T[j], 0])), y=l.iloc[i_u.values.T[j], 0])):
                for s in test_index:
    ...
    for j in range(runs_n * runs_cv_folds):
        ...
        if lp.feature_selection == 'lasso':
            fi[j] = compute_feature_importance(LassoCV(alphas=lp.fs_grid[0], cv=lp.cv_folds, n_jobs=-1).fit(
                f.loc[i_tr[j] & i_u[j // runs_cv_folds], fi[j].feat_sel].values,
                l[i_tr[j] & i_u[j // runs_cv_folds]].values.flatten().astype('int')), feat, fi[j].feat_sel,
                lp.feature_selection)
        elif lp.feature_selection == 'enet':
            fi[j] = compute_feature_importance(
                ElasticNetCV(alphas=lp.fs_grid[0], l1_ratio=lp.fs_grid[1], cv=lp.cv_folds, n_jobs=-1).fit(
                    f.loc[i_tr[j] & i_u[j // runs_cv_folds], fi[j].feat_sel].values,
                    l[i_tr[j] & i_u[j // runs_cv_folds]].values.flatten().astype('int')), feat, fi[j].feat_sel,
                lp.feature_selection)

        if lp.learner_type == 'rf':
            clf.append(RandomForestClassifier(n_estimators=500, max_depth=None, min_samples_split=2, n_jobs=-1).fit(
                f.loc[i_tr[j] & i_u[j // runs_cv_folds], fi[j].feat_sel].values,
            ...
            clf.append(
                GridSearchCV(
                    ...
                    lp.cv_grid,
                    cv=StratifiedKFold(n_splits=lp.cv_folds, shuffle=True),
            ...
            clf.append(ElasticNetCV(alphas=lp.cv_grid[0], l1_ratio=lp.cv_grid[1], cv=lp.cv_folds, n_jobs=-1).fit(
                f.loc[i_tr[j] & i_u[j // runs_cv_folds], fi[j].feat_sel].values,
                l[i_tr[j] & i_u[j // runs_cv_folds]].values.flatten().astype('int')))
        if (lp.learner_type == 'rf') | (lp.learner_type == 'svm'):
            p_es.append(
