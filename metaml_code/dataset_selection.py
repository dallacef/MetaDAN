...
def main(args):
    ...
    pf = pd.DataFrame()
    if par['select']:
        pf = pd.concat([pf, pd.DataFrame([s.split(':') for s in par['select'].split(',')],
                                         index=['select'] * (par['select'].count(',') + 1))], axis=0)
    if par['remove']:
        pf = pd.concat([pf, pd.DataFrame([s.split(':') for s in par['remove'].split(',')],
                                         index=['remove'] * (par['remove'].count(',') + 1))], axis=0)
    if par['include']:
        pf = pd.concat([pf, pd.DataFrame([s.split(':') for s in par['include'].split(',')],
                                         index=['include'] * (par['include'].count(',') + 1))], axis=0)
    if par['exclude']:
        pf = pd.concat([pf, pd.DataFrame([s.split(':') for s in par['exclude'].split(',')],
                                         index=['exclude'] * (par['exclude'].count(',') + 1))], axis=0)
    ...
    f.drop(f.loc[:, feat].columns[f.loc[:, feat].max().astype('float') == f.loc[:, feat].min().astype('float')], axis=1,
           inplace=True)

    if par['out_f']:
        if par['tout']:
            f.to_csv(par['out_f'], sep='\t', header=False, index=True, lineterminator='\n')
        else:
